...
from collections import defaultdict
from sklearn.metrics import precision_recall_curve, f1_score, recall_score, average_precision_score
...
def train_predictor(embedding_model, support_set, epochs, learning_rate, use_entropy_regularization=True):
    ...
    torch.save(predictor.state_dict(), 'models/predictor_res18.pth')
...
def test(predictor, test_set):
    ...
    for i, class_id in enumerate(np.unique(all_labels)):
        ...
        roc_auc = auc(fpr, tpr)
        aucs.append(roc_auc)
        # plt.plot(fpr, tpr, alpha=0.3, label=f'Class {class_id} (AUC = {roc_auc:.2f})')  # Plot class-specific ROC curve

        tpr_interp = np.interp(mean_fpr, fpr, tpr)  # Interpolate tpr to common fpr points
        tpr_interp[0] = 0.0  # Ensure perfectly starts at 0
        tprs.append(tpr_interp)

    mean_tpr = np.mean(tprs, axis=0)  # Average of interpolated TPRs
    mean_tpr[-1] = 1.0  # Ensure perfectly ends at 1
    mean_auc = auc(mean_fpr, mean_tpr)  # Calculate area under the mean ROC curve

    plt.plot(mean_fpr, mean_tpr, color='red', label=f'Mean ROC (AUC = {mean_auc:.2f})', lw=2, alpha=0.8)
...
emb_model = load_model('/home/jiarj2402/1003/models/emb_model_res18_cos.pth', embedding_dim=128)

# Train the predictor
train_predictor(emb_model, support_set, epochs=predictor_epochs, learning_rate=predictor_learning_rate)

embedding_model = FSCEmbNet(embedding_dim=128)
...
predictor_state_dict = torch.load('models/predictor_res18.pth')
